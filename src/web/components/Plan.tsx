          ...
          <div style={{ maxWidth: 1080, margin: "0 auto", padding: "0 24px 14px" }}>
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 13.5, color: "#3D6651", lineHeight: 1.5, marginTop: 2 }}>
                  {state.revision.summary}
                </div>
              </div>
              <button
                className="tc-success-close"
                onClick={() => dispatch({ type: "DISMISS_REVISION" })}
                style={{
      ...
      >
        <div ref={canvasRef} onClick={handleCanvasClick} style={canvasStyle}>
          <div data-annot="Header" className={annotClass.header} style={annotationStyle({ marginTop: -6 })}>
            ...
            <p style={{ fontSize: 17.5, lineHeight: 1.6, color: "#4D4D49", margin: 0, maxWidth: "62ch" }}>
              {plan.summary}
            </p>
          </div>
          ...
          <div
            data-annot="Audience & goals"
            className={annotClass.audience}
            style={annotationStyle({ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 28 })}
          >
            <div>
              <div
                style={{
          ...
          <div data-annot="Milestones" className={annotClass.milestones} style={annotationStyle({ marginBottom: 6 })}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
              {plan.milestones.map((milestone) => (
                <div
                  key={milestone.phase}
                  style={{ border: `1px solid ${colors.line}`, borderRadius: 12, padding: 16, background: "#FBFBF9" }}
                >
                  <div style={{ fontSize: 14, fontWeight: 600, letterSpacing: "-0.01em", marginBottom: 9 }}>
                    {milestone.phase}
                  </div>
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    {milestone.items.map((item) => (
                      <div key={item} style={{ fontSize: 13, color: colors.muted, lineHeight: 1.4 }}>
                        {item}
                      </div>
                    ))}
                  </div>
        ...
        <div style={{ flex: "0 0 296px", position: "sticky", top: 88 }}>
            ...
            {state.comments.length ? (
                ...
                {state.comments.map((comment) => (
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 14, lineHeight: 1.45, color: comment.addressed ? "#9A9A94" : "#2D2D29" }}>
                        {comment.text}
                      </div>
                    </div>
                    <button
              ...
              <div style={{ padding: "30px 20px", textAlign: "center" }}>
                <div style={{ fontSize: 13.5, color: colors.soft, lineHeight: 1.55 }}>
                  No comments yet. Click <b style={{ color: "#5B5B57" }}>Comment</b>, then click anywhere on the plan to
                  pin feedback.
                </div>
              </div>
            )}
          ...
          <button
            ...
            onClick={() => dispatch({ type: "BACK_TO_INTERVIEW" })}
            style={{
              marginTop: 12,
              width: "100%",
              padding: 11,
